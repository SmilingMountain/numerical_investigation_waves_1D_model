...
%% initialise variables - NON-LINEAR SYSTEM WITH OROGRAPHY, OPEN BOUNDARIES
...
for m = 1:4*L+3
    ...
    if abs(x(m)./L-0.5) <= 1.0./9.0
        hb(m,1) = h0.*9.^4.*(((x(m)./L-0.5)).^2-(1.0./9.0).^2).^2;
    else
        hb(m,1) = 0;
    end
    hprime(m,1) = 0;
...
for n=2:2
    ...
    for m = 3:4*L+1
        ...
        hprime(m,n)=hprime(m,n-1)-(dt./dx)*((U+(uprime(m,n-1)))*0.5.*((hprime(m+1,n-1)-hprime(m-1,n-1))-(hb(m+1,1)-hb(m-1,1)))+(H+(hprime(m,n-1))-hb(m,1))*(uprime(m+1,n-1)-uprime(m,n-1)));
...
for n=3:steps
    ...
    for m = 3:4*L+1
        ...
        hprime(m,n)=hprime(m,n-2)-(2.0.*dt./dx)*((U+(uprime(m,n-1))).*0.5.*((hprime(m+1,n-1)-hprime(m-1,n-1))-(hb(m+1,1)-hb(m-1,1)))+(H+(hprime(m,n-1))-hb(m,1)).*(uprime(m+1,n-1)-uprime(m,n-1)));
        Enonlin=((H+hprime(m,n))*(((uprime(m,n))^2)/2))+(g*((hprime(m,n))^2)/2); %Energy (nonlinear)
        En=En+Enonlin;
    end
